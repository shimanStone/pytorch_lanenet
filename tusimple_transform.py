#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2022/5/26 15:30
# @Author  : shiman
# @File    : tusimple_transform.py
# @describe:  tusimple 数据标签生成
...
def process_json_file(json_file_path, src_dir, ori_dst_dir, binary_dst_dir, instance_dst_dir):
    """
    :generate binary_image and instance_image
...
def process_tusimple_dataset(src_dir, val_tag, test_tag):
    ...
    for json_label_path in glob.glob('{:s}/*/label*.json'.format(src_dir)):
        ...
        if not ops.exists(ops.join(training_folder_path, json_label_name)):
    ...
    for json_label_path in glob.glob('{:s}/*/test_label*.json'.format(src_dir)):
        ...
        if not ops.exists(ops.join(testing_folder_path, json_label_name)):
    ...
    for json_label_path in glob.glob('{:s}/*.json'.format(training_folder_path)):
        process_json_file(json_label_path, f'{src_dir}/train_set', gt_image_dir, gt_binary_dir, gt_instance_dir)
    ...
    if test_tag:
        ...
        for json_label_path in glob.glob('{:s}/*.json'.format(testing_folder_path)):
            process_json_file(json_label_path, f'{src_dir}/test_set', gt_image_dir_test, gt_binary_dir_test, gt_instance_dir_test)
...
if __name__ == '__main__':
    ...
    root_dir = r'E:\data\lanenet'
    val, test = True, True
    process_tusimple_dataset(root_dir, val, test)
