...
class DiscriminativeLoss(_Loss):
    ...
    # def _discriminative_loss(self, embedding, seg_gt):
    #     bs, embed_dim, h, w = embedding.shape
    #
    #     var_loss = torch.tensor(0, dtype=embedding.dtype, device=embedding.device)
    #     dist_loss = torch.tensor(0, dtype=embedding.dtype, device=embedding.device)
    #
    #     for b in range(bs):
    #         embedding_b = embedding[b]  # shape: embed_dim, h*w
    #         seg_gt_b = seg_gt[b]  # shape: h*w
    #
    #         labels = torch.unique(seg_gt_b)
    ...
    #             dist_loss = var_loss + _nonsense * _zero
    #             continue
    #
    #         centroid_means = []
    #
    #         for lane in labels:
    #             seg_mask_i = (seg_gt_b == lane)
    #
    #             if not seg_mask_i.any():
    #                 continue
    #
    #             embedding_i = embedding_b[seg_mask_i]
    #
    #             mean_i = torch.mean(embedding_b, dim=0)
    #             centroid_means.append(mean_i)
    #             # 方差损失
    #             var_loss = var_loss + torch.mean(F.relu(
    #                 torch.norm(embedding_i - mean_i, dim=1) - self.delta_var)**2) / num_lanes
    #         # shape : num_lanes, embed_dim
    #         centroid_means = torch.stack(centroid_means)
    #
    #         if num_lanes > 1:
    #             centroid_mean1 = centroid_means.reshape(-1, 1, embed_dim)
    #             centroid_mean2 = centroid_means.reshape(1, -1, embed_dim)
    #             # shape: num_lanes, num_lanes
    #             dist = torch.norm(centroid_mean1 - centroid_mean2, dim=2)
    #             dist = dist + torch.eye(num_lanes, dtype=dist.dtype, device=dist.device) * self.delta_dist
    #             #
